...
class LivekitProxyServlet(DirectServeJsonResource):
    ...
    async def _async_render(self, request: SynapseRequest):
        print("REQUEST", request.path, request.method, request.requestHeaders, request.content.read())
        ...
        if about_statusevents:
            ...
            resp: IResponse = await http_client.request(request.method.decode(),
                                                        f"{self.config.get('livekit_client_url', 'http://localhost:7880')}/{'/'.join(request.path.decode().split('/')[4:])}",
            ...
            #     logger.error(f"Got a non-json response from Livekit: '{resp_content}'")
